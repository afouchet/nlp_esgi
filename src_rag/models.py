...
class RAG:
    def __init__(self, chunk_size=256, overlap=24):
    ...
    def _compute_chunks(self, texts):
        return sum(
            (chunk_markdown_with_overlap(txt, chunk_size=self._chunk_size, overlap=self._overlap) for txt in texts),
            [],
        )
...
def chunk_markdown_with_overlap(md_text: str, chunk_size: int = 128, overlap: int = 24) -> list[dict]:
